...
PlasmoidItem {
    ...
    // No Plasma dialog frame around the popup — we draw our own rounded box
    // (FullRepresentation) like the iOS folders, so only one rounded border shows.
    Plasmoid.backgroundHints: PlasmaCore.Types.NoBackground

    // HasH custom category groups [{name, slug}] and the currently-selected one,
    // so app delegates can offer Add-to / Remove-from group on right-click.
    property var hashGroups: []
    property string currentHashSlug: ""
