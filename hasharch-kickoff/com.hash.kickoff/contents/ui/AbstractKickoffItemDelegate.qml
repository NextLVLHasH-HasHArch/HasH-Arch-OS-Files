...
T.ItemDelegate {
    ...
    function openActionMenu(x = undefined, y = undefined) {
        ...
        // HasH-Arch: add "Add to Group ▸" (and "Remove from this Group" when a
        // custom category is open) so apps are managed from the RIGHT-side menu.
        const hashGroups = kickoff.hashGroups || [];
        const hashRef = model.favoriteId || model.url || "";
        if (hashGroups.length > 0 && hashRef) {
            if (!actions) { actions = []; }
            const subs = hashGroups.map(g => ({
                text: g.name, actionId: "_hashAddTo", actionArgument: g.slug + "::" + hashRef
            }));
            actions.push({ type: "separator" });
            actions.push({ text: i18n("Add to Group"), icon: "list-add", subActions: subs });
            if (kickoff.currentHashSlug) {
                actions.push({ text: i18n("Remove from this Group"), icon: "list-remove",
                    actionId: "_hashRemoveFrom", actionArgument: kickoff.currentHashSlug + "::" + hashRef });
            }
        }
