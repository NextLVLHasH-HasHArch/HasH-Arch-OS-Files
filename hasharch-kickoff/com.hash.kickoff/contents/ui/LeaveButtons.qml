...
RowLayout {
    ...
    FilteredModel {
        ...
        // hide these actions from the power menu entirely
        readonly property var hiddenIds: ["switch-user", "logout", "suspend"]
        function isHidden(sourceRow, sourceParent) {
            const role = sourceModel.KItemModels.KRoleNames.role("favoriteId");
            const fid = String(sourceModel.data(sourceModel.index(sourceRow, 0, sourceParent), role));
            return hiddenIds.includes(fid);
        }
        filterRowCallback: (sourceRow, sourceParent) => {
            if (isHidden(sourceRow, sourceParent)) {
                return false;
            }
            return root.__layout.collapseActionButtons
                ? true
                : !systemFavoritesContainsRow(sourceRow, sourceParent);
        }
    }
