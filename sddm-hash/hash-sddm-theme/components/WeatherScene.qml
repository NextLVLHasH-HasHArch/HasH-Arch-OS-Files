...
Item {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // target look: "Thu 21 May" (abbreviated day, day-of-month, abbreviated month)
            scene.clockDate = Qt.formatDate(d, "ddd d MMM");
    ...
    Column {
        ...
        y: scene.height * 0.13
        spacing: Math.round(scene.height * 0.004)
        // NOTE: no `layer.enabled`/DropShadow here. A GPU layer effect on the
        // wallpaper containment can fail in plasmashell and blank the WHOLE
        // wallpaper (black screen). Each Text uses a layer-free `Text.Raised`
        // shadow (styleColor) for readability instead.
        Text {                                                   // date line, above the time ("Thu 21 May")
            anchors.horizontalCenter: parent.horizontalCenter
            horizontalAlignment: Text.AlignHCenter
            text: scene.clockDate
            color: "#F5F5F7"; opacity: 0.75   // Athens Gray (translucent)
            style: Text.Raised; styleColor: "#C0000000"
            font.pixelSize: Math.round(scene.height * 0.034)
            font.weight: Font.Medium
            font.letterSpacing: Math.round(scene.height * 0.0035)
        ...
        Text {                                                   // BIG bold time
            ...
            color: "#F5F5F7"; opacity: 0.75   // Athens Gray (translucent)
            style: Text.Raised; styleColor: "#C0000000"
            font.pixelSize: Math.round(scene.height * 0.20)
            font.weight: Font.Bold
            font.letterSpacing: Math.round(scene.height * 0.004)
        }
        Item { width: 1; height: Math.round(scene.height * 0.022); visible: scene.showInfo && scene.hasInfo }
        Text {                                                   // location
            anchors.horizontalCenter: parent.horizontalCenter
            horizontalAlignment: Text.AlignHCenter
            ...
            color: "#F5F5F7"; opacity: 0.75   // Athens Gray (translucent)
            style: Text.Raised; styleColor: "#C0000000"
            font.pixelSize: Math.round(scene.height * 0.028)
            font.weight: Font.Medium
            font.letterSpacing: Math.round(scene.height * 0.0015)
        }
        Text {                                                   // temperature, below the location
            ...
            color: "#F5F5F7"; opacity: 0.75   // Athens Gray (translucent)
            style: Text.Raised; styleColor: "#C0000000"
            font.pixelSize: Math.round(scene.height * 0.060)
