...
PlasmaCore.ColorScope {
    ...
    // HasH 2FA: set once the password has been submitted, so a following
    // promptForSecret is treated as the verification-code prompt (not the password).
    property bool passwordSubmitted: false

    // HasH 2FA: returns true if the secret prompt text looks like a TOTP/2FA
    // verification-code request rather than the initial password prompt.
    ...
    Connections {
        ...
        // Modern Plasma 6 greeter API (matches stock LockScreenUi.qml).
        // kind != 0 means a non-interactive authenticator (e.g. fingerprint) failed;
        // ignore those so we don't disturb the password/2FA interactive flow.
        function onFailed(kind) {
            ...
            mainBlock.reset2FA();
            // Mirror stock: restart the interactive PAM conversation after a failure.
            root.clearPassword();
            authenticator.startAuthenticating();
        }

        function onSucceeded() {
            // Mirror stock: an interactive unlock (we had a prompt) closes the greeter.
        ...
        // Echo-on prompt (rare for PAM here); just surface the text.
        function onPromptChanged() {
            mainBlock.mainPasswordBox.forceActiveFocus();
        }

        // Secret prompt. PAM (with pam_google_authenticator in /etc/pam.d/kde)
        // fires this for the FIRST secret (the password) and then AGAIN for the
        // SECOND secret (the verification code). We distinguish them two ways:
        //  - passwordSubmitted: the password was already responded-to, so the
        //    next secret prompt must be the 2FA code; and/or
        //  - the prompt text itself looks like a code/verification request.
        function onPromptForSecretChanged() {
            if (lockScreenUi.passwordSubmitted || lockScreenUi.isCodePrompt(authenticator.promptForSecret)) {
                mainBlock.activate2FA();
            } else {
                // First secret prompt: the password. Sweet's field has no
                // show/hide toggle, so just (re)focus it for entry.
                mainBlock.mainPasswordBox.forceActiveFocus();
            }
        }
    }
    ...
    MouseArea {
        ...
        onUiVisibleChanged: {
            ...
            // Mirror stock: (re)start the PAM conversation when the UI is shown,
            // so the first secret (password) prompt fires onPromptForSecretChanged.
            authenticator.startAuthenticating();
        }
        onBlockUIChanged: {
            if (blockUI) {
        ...
        StackView {
            ...
            initialItem: MainBlock {
                ...
                // Modern API: answer the current (password) secret prompt.
                onLoginRequest: {
                    root.notification = ""
                    lockScreenUi.passwordSubmitted = true   // HasH 2FA: next secret prompt is the code
                    authenticator.respond(password)
                }

                // HasH 2FA: send the verification code back to PAM via the greeter
                // (answers the SECOND secret prompt issued by pam_google_authenticator).
                onRespondCode: {
                    root.notification = ""
