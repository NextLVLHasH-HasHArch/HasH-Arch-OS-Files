...
namespace SDDM {
    ...
    void GreeterProxy::respondToPrompt(const QString &answer) {
        // HasH 2FA: hand the popup's code back to the daemon to satisfy the PAM prompt
        ...
        while (input.device()->bytesAvailable()) {
            ...
            switch (DaemonMessages(message)) {
                ...
                case DaemonMessages::PromptRequest: {
                    // HasH 2FA: the daemon is relaying an extra PAM prompt (e.g. the
                    // verification code) — surface it to the theme's popup.
                    QString message;
                    bool secret = true;
                    input >> message >> secret;
                    emit promptRequested(message, secret);
                }
                break;
                default: {
                    // log message
