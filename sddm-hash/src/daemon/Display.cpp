...
namespace SDDM {
        ...
        m_greeter(new Greeter(this))
    {
        ...
        connect(m_socketServer, &SocketServer::promptResponse, this, &Display::slotPromptResponse);  // HasH 2FA
    ...
    void Display::slotRequestChanged() {
        AuthRequest *request = m_auth->request();
        // Fill the prompts we recognise (user / password). Relay the first prompt we
        // DON'T recognise (e.g. a 2FA verification code) to the greeter and defer
        // done() until it answers via slotPromptResponse().
        AuthPrompt *relay = nullptr;
        for (AuthPrompt *prompt : request->prompts()) {
            switch (prompt->type()) {
            case AuthPrompt::LOGIN_USER:
                prompt->setResponse(qPrintable(m_auth->user()));
                break;
            case AuthPrompt::LOGIN_PASSWORD:
                prompt->setResponse(qPrintable(m_passPhrase));
                break;
            default:
                if (!relay)
                    relay = prompt;
        ...
        if (relay && m_socket) {
            m_pendingPrompt = relay;
            m_socketServer->promptRequest(m_socket, relay->message(), true /* hidden input */);
            return;   // done() is called once the greeter replies
        }
        request->done();
    }

    void Display::slotPromptResponse(QLocalSocket *socket, const QString &answer) {
        // HasH 2FA: the greeter answered the relayed prompt (e.g. the verification code)
        if (socket != m_socket || !m_pendingPrompt)
            return;
        m_pendingPrompt->setResponse(answer.toUtf8());
