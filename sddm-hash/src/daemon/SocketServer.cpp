...
namespace SDDM {
    ...
    void SocketServer::readyRead() {
        ...
        while(socket->bytesAvailable()) {
            ...
            switch (GreeterMessages(message)) {
                ...
                break;
                case GreeterMessages::PromptResponse: {
                    // HasH 2FA: the greeter is answering a relayed PAM prompt (the code)
                    QString answer;
                    input >> answer;
                    emit promptResponse(socket, answer);
                }
                break;
                default: {
                    // log message
    ...
    void SocketServer::promptRequest(QLocalSocket *socket, const QString &message, bool secret) {
        // HasH 2FA: relay an extra PAM prompt (verification code, …) to the greeter
        SocketWriter(socket) << quint32(DaemonMessages::PromptRequest) << message << secret;
    }
}
